...
public class FinancesScreen extends VBox {
    ...
    private void loadDataFromDatabase() {
        ...
        // Φιλτράρισμα: Μόνο τα έγκυρα/εγκεκριμένα έξοδα του δωματίου
        String sql = "SELECT * FROM bills WHERE room_id = ? AND approval_status = 'Accepted'";
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    allBills.add(new Bill(
                        ...
                        rs.getString("approval_status") // Προσθήκη ανάγνωσης εδώ
                    ));
                }
            }
    ...
    private void handlePayBill() {
        ...
        new ConfirmationScreen("Pay Bill", "Mark as paid?", () -> {
            ...
            // Εξασφαλίζουμε ότι θα ενημερωθούν/διαγραφούν μόνο τα στοιχεία που ανήκουν στο συγκεκριμένο δωμάτιο
            ...
            try (Connection conn = DatabaseManager.getConnection()) {
                ...
                // 1. Update status στο bills table
                ...
                // 2. Remove από το calendar table
                ...
                conn.commit(); // Οριστικοποίηση αλλαγών
                
                // Ενημέρωση της λίστας στο UI
