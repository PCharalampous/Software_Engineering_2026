...
public class ChoreDistributionService {
    ...
    public void distributeWeeklyChores() {
        ...
        try (Connection conn = DatabaseManager.getConnection()) {
            ...
            // 1. Έλεγχος αν υπάρχουν pending
        	String checkPendingSql = "SELECT COUNT(*) FROM chores WHERE room_id = ? AND chore_status = 'Pending' AND assignee != 'Unassigned'";
        	try (PreparedStatement psCheck = conn.prepareStatement(checkPendingSql)) {
        	    psCheck.setInt(1, roomId);
        	    try (ResultSet rs = psCheck.executeQuery()) {
        	        if (rs.next() && rs.getInt(1) > 0) {
        	            System.out.println("Cannot shuffle! Unfinished tasks still exist.");
        	            javafx.application.Platform.runLater(() -> {
        	                javafx.scene.control.Alert alert = new javafx.scene.control.Alert(
        	                    javafx.scene.control.Alert.AlertType.WARNING,
        	                    "There are still pending chores! Complete them before redistributing.",
            ...
            // 2. ΑΛΛΑΓΗ: Αφαιρέθηκε το insertDefaultChores — δεν θέλουμε hardcoded chores.
            // Αν δεν υπάρχουν chores, ενημερώνουμε τον χρήστη να τα προσθέσει μόνος του.
            ...
            if (choreIds.isEmpty()) {
                ...
                // ΑΛΛΑΓΗ: Ειδοποίηση στον χρήστη να προσθέσει chores με το "+"
                javafx.application.Platform.runLater(() -> {
                    javafx.scene.control.Alert alert = new javafx.scene.control.Alert(
