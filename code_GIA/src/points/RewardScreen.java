...
public class RewardScreen extends VBox {
    ...
    private void handleVote(Reward r, boolean approve) {
        ...
        String logVoteSql = "INSERT INTO chore_reports (chore_id, title, description) " +
                            "VALUES ((SELECT chore_id FROM chores LIMIT 1), ?, ?)";
        ...
        String updateRewardSql = approve ? 
            ...
            try (PreparedStatement psLog = conn.prepareStatement(logVoteSql)) {
                psLog.setString(1, (approve ? "REWARD_APPROVE_" : "REWARD_REJECT_") + currentUsername);
                psLog.setString(2, "RewardID:" + r.getRewardId()); 
                psLog.executeUpdate();
            }

            try (PreparedStatement psUpdate = conn.prepareStatement(updateRewardSql)) {
                psUpdate.setInt(1, r.getRewardId());
                psUpdate.setInt(2, pointSidebar.getCurrentRoomId()); // Ασφάλεια εδώ!
